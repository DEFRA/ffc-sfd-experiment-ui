...
const calculateScore = () => {
  let scoreArray = []
  document
    .querySelectorAll('form[id="items-list"] input.govuk-input--width-3')
    .forEach(input => {
      if (input.value !== '') {
        const score = document
          .querySelector(`div[id="${input.name}-score"]`)
          .innerHTML.split(':')[1]
          .trim()
  ...
  if (scoreArray.length > 0) {
    ...
    const itemScoreTotal = scoreArray.reduce(
...
const calculateGrantTotal = () => {
  const calculateGrantTotal = [
    ...document.querySelectorAll('div[id$="-total"]')
  ].reduce(
    (previousValue, currentValue) =>
      previousValue +
      parseInt(currentValue.innerHTML.substring(1).replace(',', ''), 10),
    0
  )
  ...
  document.querySelector(
    'p[id="grant-total"]'
  ).innerHTML = `£${grantTotalDisplayValue}`
}

const calculateItemTotal = (equipmentId, numberOfItems) => {
  const itemValue = document.querySelector(
    `div[id="${equipmentId}-price"]`
  ).innerHTML
  let newTotal
  if (numberOfItems === 0) {
    newTotal = '0.00'
  } else {
    newTotal = formatUKCurrency(
      parseInt(itemValue.substring(1).replace(',', ''), 10) * numberOfItems
    )
  }
...
export function itemsListChecker () {
  ...
  if (elements) {
    const listOfInputs = document.querySelectorAll(
      'form[id="items-list"] input.govuk-input--width-3'
    )
    listOfInputs.forEach(input => {
      ...
      if (input.value !== '') {
        console.log(
          'Page loaded with a value already in input most likely due to an error on submission',
          input.name
        )

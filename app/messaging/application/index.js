...
async function getGrants (sessionId, msgQueueSuffix, userData, grantID = null) {
  console.log(
    '[MADE IT TO MESSAGE]',
    getGrantReqResQueueAddress(msgQueueSuffix),
    'PPPPPPPPPPP'
  )
  const msgBody = grantID ? { grantID: grantID } : userData
  const grantMsgType = grantID
    ? fetchApplicationRequestMsgType
    : fetchGrantsRequestMsgType
  const { grantRequestQueueAddress, grantResponseQueueAddress } =
    getGrantReqResQueueAddress(msgQueueSuffix)
  await sendMessage(msgBody, grantMsgType, grantRequestQueueAddress, {
    sessionId
  })
...
async function grantSubmitted (msgQueueSuffix, msgBody) {
  ...
  const { grantRequestQueueAddress } =
    getGrantReqResQueueAddress(msgQueueSuffix)
  await sendMessage(
    msgBody,
    fetchApplicationRequestMsgType,
    grantRequestQueueAddress
  )
...
const getGrantReqResQueueAddress = msgQueueSuffix => {
  ...
  return {
    grantRequestQueueAddress: grantRequestQueueAddress,
    grantResponseQueueAddress: grantResponseQueueAddress
  }
}
